...
class tbMenusElements extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_tbmenuselements';
	var $table_index= 'tbmenuselementsid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_tbmenuselementscf', 'tbmenuselementsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_tbmenuselements', 'vtiger_tbmenuselementscf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_tbmenuselements' => 'tbmenuselementsid',
		'vtiger_tbmenuselementscf'=>'tbmenuselementsid');
	...
	var $list_fields = Array (
	    ...
	    'LBL_TBMENUSELEMENTS_MENU' => Array('vtiger_tbmenuselements', 'menu'),
	    'LBL_TBMENUSELEMENTS_TITLE' => Array('vtiger_tbmenuselements', 'title'),
	    'LBL_TBMENUSELEMENTS_PRICE' => Array('vtiger_tbmenuselements', 'price'),
	    'LBL_TBMENUSELEMENTS_CURRENCY' => Array('vtiger_tbmenuselements', 'currency'),
	    'LBL_TBMENUSELEMENTS_ACTIVE' => Array('vtiger_tbmenuselements', 'active'),
		'Assigned To' => Array('crmentity','smownerid')
	);
	var $list_fields_name = Array (
	    /* Format: Field Label => fieldname */
	    'LBL_TBMENUSELEMENTS_MENU' => 'menu',
	    'LBL_TBMENUSELEMENTS_TITLE' => 'title',
	    'LBL_TBMENUSELEMENTS_PRICE' => 'price',
	    'LBL_TBMENUSELEMENTS_CURRENCY' => 'currency',
	    'LBL_TBMENUSELEMENTS_ACTIVE' => 'active',
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
 	...
 	function save_module($module) {
 	    ...
 	    // orden por defecto
 	    if ((int)$this->column_fields['order_element'] <= 0)
 	    {
 	        $v_query = "SELECT (MAX(order_element) + 1) as num FROM vtiger_tbmenuselements WHERE category = ? ";
 	        ...
 	        if ($v_res)
 	        {
 	            ...
 	            if ($v_max > 0)
 	            {
 	                $v_query = "UPDATE vtiger_tbmenuselements 
                        SET order_element = ?
                        WHERE tbmenuselementsid = ?";
 	...
 	function saveAttachment($id, $module) {
 	    ...
 	    foreach ($_REQUEST as $i => $v) {
 	        ...
 	        if ((strpos($i, 'bwmdeleteView_') !== false) && ($v!='')) {
 	            ...
 	            $result = $adb->pquery("SELECT * FROM vtiger_tbmenuselements WHERE tbmenuselementsid = ?",array($id));
 	            while($row = $adb->fetchByAssoc($result))
 	            {
 	                ...
 	                if (($attachid!=0) && ($attachid!='')) {
 	                    ...
 	                    $adb->pquery("UPDATE vtiger_tbmenuselements set ".$v." = '' WHERE tbmenuselementsid = ?",array($id));
 	                }
 	            }
 	        }
 	    }
 	    ...
 	    if ($_FILES) {
 	        ...
 	        foreach ($_FILES as $fileindex => $files) {
 	            if ($files['name'] != '' && $files['size'] > 0) {
 	                ...
 	                {
 	                    $v_query = "UPDATE vtiger_tbmenuselements SET $fileindex = ? WHERE tbmenuselementsid = ?";
