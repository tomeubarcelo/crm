...
class tbqr extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_tbqr';
	var $table_index= 'tbqrid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_tbqrcf', 'tbqrid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_tbqr', 'vtiger_tbqrcf');
	...
	var $list_fields = Array (
	    ...
	    'LBL_TBQR_TITLE_QR' => Array('vtiger_tbqr', 'title_qr'),
	    'LBL_TBQR_TIPO_QR' => Array('vtiger_tbqr', 'tipo_qr'),
	    'LBL_TBQR_TOKEN' => Array('vtiger_tbqr', 'token'),
	    'LBL_TBQR_URL_GENERADA' => Array('vtiger_tbqr', 'url_generada'),
		'Assigned To' => Array('crmentity','smownerid')
	);
	var $list_fields_name = Array (
	    /* Format: Field Label => fieldname */
	    'LBL_TBQR_TITLE_QR' => 'title_qr',
	    'LBL_TBQR_TIPO_QR' => 'tipo_qr',
	    'LBL_TBQR_TOKEN' => 'token',
	    'LBL_TBQR_URL_GENERADA' => 'url_generada',
	    'Assigned To' => 'smownerid'
	);

	// Make the field link to detail view
	var $list_link_field = 'title_qr';
	...
	var $popup_fields = Array ('title_qr');

	// For Alphabetical search
	var $def_basicsearch_col = 'title_qr';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'title_qr';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('title_qr','assigned_user_id');

	var $default_order_by = 'title_qr';
	...
	*/
	...
	function save_module($module) {
	    ...
	    if ($this->column_fields['token'] == '')
	    {
	        $v_token = $this->id.substr(md5($this->id.date('YdmiHs').rand(10000,100000)), 16, 5);
	        $v_url = $site_URL.'qr.php?t='.$v_token;
	        
	        $v_query = "UPDATE vtiger_tbqr SET token = ?, url_generada = ? WHERE tbqrid = ? AND (token = '' OR token IS NULL) ";
	        $adb->pquery($v_query, array($v_token, $v_url, $this->id));
	        
	        // create QR
	        file_get_contents($site_URL.'qr.php?t='.$v_token.'&url='.base64_encode($v_url));
	    }
	    ...
	    if ($_FILES) {
	        ...
	        foreach ($_FILES as $fileindex => $files) {
	            if ($files['name'] != '' && $files['size'] > 0) {
	                ...
	                {
	                    $v_query = "UPDATE vtiger_tbqr SET $fileindex = ? WHERE tbqrid = ?";
