...
{strip}
	...
	<div class="fileUploadContainer text-left">
		<div class="fileUploadBtn btn btn-sm btn-primary">
			...
			<input type="file" id="{$MODULE}_editView_fieldName_{$FIELD_MODEL->get('name')}" class="inputElement {if $MODULE eq 'ModComments'} multi {/if} " name="{if $MODULE eq 'ModComments'}{$FIELD_MODEL->getFieldName()}[]{else}{$FIELD_MODEL->getFieldName()}{/if}"
		...
		<span class="uploadFileSizeLimit fa fa-info-circle" data-toggle="tooltip" data-placement="top" title="{vtranslate('LBL_MAX_UPLOAD_SIZE',$MODULE)} {$MAX_UPLOAD_LIMIT_MB} {vtranslate('MB',$MODULE)}">
			<span class="maxUploadSize" data-value="{$MAX_UPLOAD_LIMIT_BYTES}"></span>
				&nbsp;<a onclick="delete_image('{$FIELD_MODEL->get('name')}');" class="editAction fa fa-trash"></a><input type="hidden" id="bwmdeleteView_{$FIELD_MODEL->get('name')}" name="bwmdeleteView_{$FIELD_MODEL->get('name')}" value=""/>
			</span>
		<div class="uploadedFileDetails {if $IS_EXTERNAL_LOCATION_TYPE}hide{/if}">
			<div class="uploadedFileSize" name="uploadedFileSize_{$FIELD_MODEL->get('name')}"></div>
			<div class="uploadedFileName" name="uploadedFileName_{$FIELD_MODEL->get('name')}">
